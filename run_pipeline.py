...
所谓“安身立命”的根据。在这个意义上，古代的人类是“嵌入”在整体宇宙之中的。
...
还记得我前面用的那个词吗，在古代社会，人是“嵌入”这个世界里的，是和世界连为一体的。而到了现代社会，他从那么大的“母体”中被剥离出来，从此孤独地、无靠地存活在这个世界上。
...
""".strip()
    ...
    data = run_pipeline(
        book="西方现代思想讲义",
        author="刘擎",
        chapter="05.为什么说“祛魅”是人类的梦醒时分",
        article_text=content,
    )
    with open("output.json", "w") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    for i, idea in enumerate(data["core_ideas"]):
        with open(f"{idea['title']}.md", "w") as f:
            f.write(idea["core_idea"])
        for j, motif in enumerate(idea["motifs"]):
            with open(f"{motif['title']}.md", "w") as f:
                f.write(motif["motif"])
        with open(f"{idea['summary']['title']}.md", "w") as f:
            f.write(idea["summary"]["summary"])
        save_as_docx(
            idea["summary"]["summary"],
            f"{idea['summary']['title']}.docx",
        )
